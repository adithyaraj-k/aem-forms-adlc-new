...
| `formwright` (IMPL build lead) | generate-schema + create-fdm + create-editable-template + create-form-component + create-adaptive-form + create-form-rules + create-form-clientlib + create-form-theme + migrate-form + create-AdaptiveFormFragment (opt-in, only on explicit user request) | IMPL build — data foundation (schema/FDM) + artifact-build phases (1, 2, 3, 4, 5, 8, 9, 11) against the DESI specs, with every field authored directly inside the form; also owns migrating/scaffolding the `ui.tests` **Playwright** harness + authoring its specs PRE-DEPLOY (no skill — sentinel only executes it post-deploy against cloud DEV) |
...
IMPL-B   formwright (build)           → needs DESI; data foundation (schema/FDM) + build phases 1,2,3,4,5,8,9,11 to the specs (fields authored directly; fragment only if explicitly requested)
...
flowchart TD
    ...
    GD -- Yes --> FW[["formwright (IMPL build) ·<br/>schema/FDM · template · component ·<br/>form (fields authored directly) · rules · clientlib · theme · migrate ·<br/>fragment (opt-in, only if explicitly requested)"]]
...
>    (1, 2, 3, 4, 5, 8, 9, 11), against the DESI specs, with every field authored directly inside
>    the form — Adaptive Form Fragments only when the user explicitly requested one.
...
IMPL-B   │ formwright                │ Data foundation (schema/FDM) + build 1,2,3,4,5,8,9,11 to the specs (fields authored directly; fragment opt-in only)
...
| Fragments are opt-in, never automatic | Every generic repeatable section (address, contact/personal details, emergency contact, declaration/consent, signature) is authored **directly inside the form's `guideContainer`** by default — inline, not as an Adaptive Form Fragment. Draftsmith does NOT flag these `source: fragment` by default, and Formwright does NOT create/reuse a fragment (`create-AdaptiveFormFragment`, `fragmentPath`) unless the user **explicitly** requested a reusable/shared fragment for this delivery. When a fragment IS explicitly requested, it must bind to a **canonical schema-agnostic shape** (`$.address.*`, `$.declaration.*`), use `blank-af-v2` (never `blank-af-v2-fragment`), and (when converting an already-deployed form) flag orphan inline nodes for Forgemaster to purge. **Reject any new form that contains a fragment or a `fragmentPath` reference when no explicit fragment request is recorded in `plan/planwright.md` (`fragments_specced`/`fragments_built` must be `0` unless `fragment_requested: true`)** |
