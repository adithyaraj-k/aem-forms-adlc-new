...
1. **Pre-req:** ensure `.aem-forms-config.yaml` exists (if missing, `ensure-forms-agents-md` runs first
   ...
   — identify `{useCaseFolder}` from the delivery INPUT first (see "Run output location" below); for a
   URL/screenshot replica it is `Use Case 1 - AEM Forms using URL or Screenshot` —
   with **all eight** folders — `plan/`, `design/`, `implement/`, `integrate/`, `deploy/`, `test/`,
   `handoffs/`, `reports/` (today's date + the primary form name; AGENTS.md → "Run output
...
- **Your end-deliverables go in `.claude/agents/runs/{runId}/plan/`** — and nowhere else.
- **Your handoff YAML goes in `.claude/agents/runs/{runId}/handoffs/planwright.yaml`.**
...
**Write this YAML to `.claude/agents/runs/{runId}/handoffs/planwright.yaml` as well as returning it** — a handoff returned in chat but not written to file does not pass the gate.
